...
// Mode-reactive: emerald + commercial framing in Business mode,
// blue + residential framing in Homes mode.
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-4 text-center">
          ...
          <p className="text-2xl font-extrabold text-orange-700">{fmt(commSavings)}</p>
        ...
        <div className={`border rounded-xl p-4 text-center ${accentBg === "bg-blue-600" ? "bg-blue-50 border-blue-200" : "bg-emerald-50 border-emerald-200"}`}>
          ...
          <p className={`text-2xl font-extrabold ${accent}`}>{fmt(totalWin)}</p>
      ...
      <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-2">
        ...
        <div className="border-t border-gray-200 pt-2 flex justify-between items-center">
          <span className="font-semibold text-gray-800">Total vs. traditional sale</span>
          <span className={`font-extrabold text-xl ${accent}`}>{fmt(totalWin)}</span>
        </div>
      </div>
...
function CommercialSellerCalc({ accent, ring }) {
  ...
  const commSaved   = price * (commPct / 100) * 1.13; // + HST
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-center">
          ...
          <p className="text-2xl font-extrabold text-amber-700">{fmt(commSaved)}</p>
        ...
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 text-center">
          ...
          <p className="text-2xl font-extrabold text-emerald-700">{fmt(totalWin)}</p>
      ...
      <div className="bg-gray-50 rounded-xl p-4 text-sm space-y-2">
        ...
        <div className="border-t border-gray-200 pt-2 flex justify-between items-center">
          <span className="font-semibold text-gray-800">Total vendor advantage</span>
          <span className="font-extrabold text-xl text-emerald-700">{fmt(totalWin)}</span>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-gray-500">
        {isBusiness
          ? "Compare a seller-financed VTB against conventional commercial financing. See what flexibility costs."
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className={`border-2 rounded-xl p-5 text-center ${isBusiness ? "bg-emerald-50 border-emerald-200" : "bg-purple-50 border-purple-200"}`}>
        ...
        <div className="bg-gray-50 border-2 border-gray-200 rounded-xl p-5 text-center">
          <p className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-1">
            {isBusiness ? "Conv. Commercial" : "Bank Mortgage"}
          </p>
      ...
      <div className="bg-gray-50 rounded-xl p-4 text-sm">
        <div className="flex justify-between mb-1">
          <span className="text-gray-600">Monthly cost difference</span>
        ...
        <p className="text-xs text-gray-500 mt-2 leading-relaxed">
          {diff > 0
            ? `A VTB at ${vtbRate}% costs ~${fmt(diff)}/month more than conventional financing at ${bankRate}%. For ${isBusiness ? "commercial deals, this premium is often worth it to avoid 60–90 day lender delays and rigid covenant requirements" : "many buyers, this is worth it to avoid bank rejection and get into homeownership now"}.`
...
function CapGainsDeferral({ accent }) {
  ...
  const estimatedTax  = taxableGain * 0.27; // ~27% marginal on taxable gain
  ...
  return (
    <div className="space-y-6">
      <p className="text-sm text-gray-500">
        A VTB lets you spread capital gains recognition over the payment term — potentially deferring a
        significant tax liability. Model your situation below (always confirm with your accountant).
      </p>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { l: "Taxable Gain (50% incl.)",   v: fmt(taxableGain),        note: "At current inclusion rate",   c: "text-orange-600"},
          { l: "Est. Tax if Lump Sum",       v: fmt(estimatedTax),       note: "~27% on taxable gain",        c: "text-red-600"   },
          { l: "Tax Deferred — Year 1",      v: fmt(totalTaxDeferral),   note: "By spreading over term",      c: "text-emerald-700"},
        ].map(({ l, v, note, c }) => (
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-xs text-amber-800 leading-relaxed">
        <strong>Important:</strong> Capital gains tax deferral via VTB (the "capital gains reserve") is a real but complex
        provision under the Income Tax Act. The structure must be set up correctly and the rules are technical.
        This calculator is illustrative only — always consult a CPA before structuring any VTB for tax purposes.
      </div>
    </div>
  );
}
...
export default function SavingsCalculator({ defaultTab, compact = false }) {
  ...
  const tabs = isBusiness
    ? [
        ...
        { id: "capgains",label: "Capital Gains Deferral"  },
      ]
    : [
        { id: "seller",  label: "For Sellers — What You Keep"  },
        { id: "buyer",   label: "For Buyers — Compare Mortgages" },
      ];
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      {/* Tabs */}
      <div className={`flex border-b border-gray-100 ${isBusiness ? "" : ""}`}>
        {tabs.map((t) => (
          <button key={t.id} onClick={() => setTab(t.id)}
            className={`flex-1 py-3.5 text-sm font-semibold transition-colors ${tab === t.id ? tabActive : tabInactive}`}>
            {t.label}
          </button>
        ))}
      </div>
