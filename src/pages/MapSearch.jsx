...
// responsive control bar. Homes mode (blue theme).
...
import {
  ...
  PenTool, ChevronLeft, ChevronRight, TrendingDown, Sparkles,
  ChevronUp, ChevronDown, List, Map as MapIcon, Columns,
} from "lucide-react";
...
const SESSION_KEY = "selfi_map_state_homes";
...
function geoCircle(cLng, cLat, rKm) {
  ...
  for (let i = 0; i <= 64; i++) {
    ...
    c.push([
      cLng + (rKm / (111.32 * Math.cos((cLat * Math.PI) / 180))) * Math.cos(a),
      cLat + (rKm / 110.574) * Math.sin(a),
    ]);
  }
...
function RadiusInput({ value, onChange }) {
  ...
  return (
    <button onClick={() => setEditing(true)}
      ...
      title="Click to type">{value}km</button>
  );
}
...
function MiniCarousel({ images }) {
  ...
  if (imgs.length === 0) return <div className="w-full h-40 bg-gray-200" />;
  return (
      ...
      {imgs.length > 1 && (
        <>
          <button onClick={(e) => { e.stopPropagation(); setIdx((i) => (i - 1 + imgs.length) % imgs.length); }}
            className="absolute left-1.5 top-1/2 -translate-y-1/2 w-7 h-7 bg-black/50 hover:bg-black/70 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity min-h-auto">
            <ChevronLeft className="w-4 h-4 text-white" />
          </button>
          <button onClick={(e) => { e.stopPropagation(); setIdx((i) => (i + 1) % imgs.length); }}
            className="absolute right-1.5 top-1/2 -translate-y-1/2 w-7 h-7 bg-black/50 hover:bg-black/70 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity min-h-auto">
...
function MapListingCard({ listing, isSelected, isHovered, onClick, onHover, onLeave, compact = false }) {
  ...
  // Compact variant for mobile card tray
  if (compact) {
    return (
      <button onClick={onClick} className={`w-64 shrink-0 bg-white rounded-xl overflow-hidden shadow-md snap-start ${
        ...
        <div className="h-28 bg-gray-100 overflow-hidden relative">
          ...
          <span className="absolute top-1.5 right-1.5 text-[9px] font-semibold px-1.5 py-0.5 rounded-full text-white min-h-auto" style={{ background: dealColor }}>
            {listing.dealType}
          </span>
        </div>
        <div className="p-3">
          <p className="font-bold text-blue-600 text-sm">{fmtCurrency(listing.price)}</p>
      ...
      <div className="flex">
        <div className="w-32 h-28 shrink-0 bg-gray-100 overflow-hidden relative">
          ...
          {listing.daysOnMarket != null && (
            <div className="absolute bottom-1.5 left-1.5 bg-black/60 text-white px-1.5 py-0.5 rounded text-[9px] font-medium backdrop-blur-sm min-h-auto">
              {listing.daysOnMarket === 0 ? "New" : `${listing.daysOnMarket}d`}
            </div>
          )}
          {priceDropped && (
            <div className="absolute top-1.5 left-1.5 bg-green-600 text-white px-1.5 py-0.5 rounded text-[9px] font-bold flex items-center gap-0.5 min-h-auto">
        ...
        <div className="flex-1 p-3 min-w-0">
          <div className="flex items-start justify-between gap-1">
            <div>
              <p className="font-bold text-blue-600 text-base">{fmtCurrency(listing.price)}</p>
              {priceDropped && <p className="text-[10px] text-gray-400 line-through">{fmtCurrency(listing.previousPrice)}</p>}
            </div>
            <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full text-white shrink-0 min-h-auto" style={{ background: dealColor }}>
              {listing.dealType}
            </span>
          </div>
          ...
          <div className="flex items-center gap-1.5 mt-2">
            {listing.propertyType && <span className="text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-600 min-h-auto">{listing.propertyType}</span>}
            {listing.yearBuilt && <span className="text-[9px] font-medium px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-600 min-h-auto">{listing.yearBuilt}</span>}
          </div>
        </div>
      </div>
    </button>
...
export default function MapSearch() {
  ...
  const [propTypes, setPropTypes] = useState(saved?.propTypes || []);
  const [minBeds, setMinBeds] = useState(saved?.minBeds ?? 0);
  ...
  useEffect(() => {
    getAllListings().then((all) => {
      const withCoords = all.map((l) => {
        ...
        return { ...l, lat: f.lat + (Math.random() - 0.5) * 0.012, lng: f.lng + (Math.random() - 0.5) * 0.018 };
      }).filter(Boolean);
  ...
  useEffect(() => {
    ...
    if (drawnPoly && drawnPoly.length >= 3) {
      const ring = [...drawnPoly, drawnPoly[0]];
  ...
  useEffect(() => {
    ...
    const zoom = map ? map.getZoom() : saved?.zoom || 10.5;
    saveMapState(SESSION_KEY, {
      center: { lng: center.lng, lat: center.lat }, zoom, view,
      pricePreset, dealTypes, propTypes, minBeds, selectedId: selected?.id || null,
    });
  }, [view, pricePreset, dealTypes, propTypes, minBeds, selected]);
  ...
  const buildGeoJSON = useCallback((data) => ({
    ...
    features: data.map((l) => ({
      ...
      properties: { id: l.id, price: l.price, previousPrice: l.previousPrice || null, dealType: l.dealType, daysOnMarket: l.daysOnMarket ?? null },
    })),
  }), []);
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      // Invisible unclustered point layer (for querying)
      map.addLayer({
  ...
  // Update cluster source when filtered changes
  useEffect(() => {
  ...
  // Sync DOM markers to unclustered points
  const syncDOMMarkers = useCallback((map) => {
    ...
    features.forEach((f) => {
      ...
      if (!visibleIds.has(id)) { visibleIds.add(id); featureMap[id] = f; }
    });
  ...
  // Clear stale markers on filter change
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const updatePreview = () => {
      ...
      if (pts.length >= 2) {
        ...
        map.getSource("draw-poly")?.setData({ type: "FeatureCollection", features: [{ type: "Feature", geometry: { type: "Polygon", coordinates: [ring] } }] });
      }
    };
    ...
    return () => {
      canvas.style.cursor = ""; map.off("click", onClick); map.off("dblclick", onDblClick);
      map.doubleClickZoom.enable(); drawPoints.current = [];
      map.getSource("draw-points")?.setData({ type: "FeatureCollection", features: [] });
    };
  }, [drawMode, mapReady]);

  // Scroll list card into view on hover
  useEffect(() => {
  ...
  const goToListing = useCallback((id) => {
    ...
    if (map) {
      ...
      saveMapState(SESSION_KEY, {
        center: { lng: center.lng, lat: center.lat }, zoom: map.getZoom(),
        view, pricePreset, dealTypes, propTypes, minBeds, selectedId: id,
      });
    }
    navigate(`/listings/${id}`);
  }, [navigate, view, pricePreset, dealTypes, propTypes, minBeds]);
  ...
  return (
    <div style={{ height: `calc(100vh - ${NAVBAR_HEIGHT}px)`, overflow: "hidden", display: "flex", flexDirection: "column" }}>
      ...
      <div ref={topBarRef} className="bg-white border-b border-gray-200 px-3 sm:px-5 py-2 sm:py-2.5 flex items-center gap-2 sm:gap-3 shrink-0 overflow-x-auto scrollbar-hide">
        ...
        <span className="text-xs sm:text-sm text-gray-400 shrink-0">{filtered.length} listings</span>

        {/* Desktop: full controls / Mobile: scrollable pill row */}
        <div className="flex items-center gap-1.5 sm:gap-2 ml-1 flex-nowrap sm:flex-wrap">
          <button onClick={locateMe} disabled={locating}
            className={`flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors shrink-0 ${
            ...
            {locating ? "..." : radiusActive ? (<>Within <RadiusInput value={radiusKm} onChange={setRadiusKm} /></>) : "Near Me"}
          </button>
          {radiusActive && !isMobile && (
            <>
              ...
              <button onClick={clearRadius} className="p-1 hover:bg-gray-100 rounded-full min-h-auto"><X className="w-4 h-4 text-gray-400" /></button>
            </>
          )}
          {!isMobile && (
            <button onClick={() => { if (!drawMode) { clearDraw(); setDrawMode(true); } }}
              className={`flex items-center gap-1.5 px-3.5 py-2 rounded-lg text-sm font-medium transition-colors shrink-0 ${
                drawMode ? "bg-green-600 text-white" : drawnPoly ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
              }`}>
              <PenTool className="w-4 h-4" />
              {drawMode ? "Click points, dbl-click finish" : drawnPoly ? "Boundary" : "Draw Area"}
            </button>
          )}
          ...
          {isMobile ? (
            <>
              <button onClick={() => setView("list")}
                className={`px-2.5 py-1.5 rounded-md text-xs font-medium transition-colors ${view === "list" ? "bg-white shadow text-gray-900" : "text-gray-500"}`}>
                <List className="w-3.5 h-3.5" />
              </button>
              <button onClick={() => setView("map")}
                className={`px-2.5 py-1.5 rounded-md text-xs font-medium transition-colors ${view === "map" ? "bg-white shadow text-gray-900" : "text-gray-500"}`}>
        ...
        <button onClick={() => setSidebarOpen(!sidebarOpen)}
          className={`flex items-center gap-1 sm:gap-1.5 px-2.5 sm:px-3.5 py-1.5 sm:py-2 rounded-lg text-xs sm:text-sm font-medium transition-colors shrink-0 ${
            activeN > 0 ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
          }`}>
          <SlidersHorizontal className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> Filters
          {activeN > 0 && <span className="bg-white text-blue-600 text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center min-h-auto">{activeN}</span>}
        </button>
      </div>

      {/* ── Main content area ── */}
      <div style={{ height: mapAreaH, position: "relative" }}>

        {/* Filter sidebar */}
        {sidebarOpen && (
          <>
            <div className="fixed inset-0 bg-black/30 z-30 sm:hidden" onClick={() => setSidebarOpen(false)} />
            <div className={`absolute top-0 right-0 bottom-0 z-40 bg-white shadow-xl border-l border-gray-200 overflow-y-auto ${
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => setSidebarOpen(false)} className="p-1 hover:bg-gray-100 rounded-full min-h-auto"><X className="w-4 h-4" /></button>
                </div>
              </div>
              <div>
                  ...
                  {PRICE_PRESETS.map((p, i) => (
                    <button key={i} onClick={() => setPricePreset(i)}
                      className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${
                        pricePreset === i ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"
                      }`}>{p.label}</button>
                  ))}
                </div>
              </div>
                  ...
                  {[0, 1, 2, 3, 4, 5].map((n) => (
                    <button key={n} onClick={() => setMinBeds(n)}
                      className={`w-10 h-10 rounded-full text-sm font-medium border transition-all ${
                        minBeds === n ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"
                      }`}>{n === 0 ? "Any" : n === 5 ? "5+" : n + "+"}</button>
                  ))}
                </div>
              </div>
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {DEAL_TYPES.map((t) => (
                    <button key={t} onClick={() => toggle(dealTypes, setDealTypes, t)}
                      className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${
                        dealTypes.includes(t) ? "text-white border-transparent" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {PROPERTY_TYPES.map((t) => (
                    <button key={t} onClick={() => toggle(propTypes, setPropTypes, t)}
                      className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${
                        propTypes.includes(t) ? "bg-blue-600 text-white border-blue-600" : "bg-white text-gray-600 border-gray-200 hover:bg-gray-50"
                      }`}>{t}</button>
                  ))}
                </div>
              </div>
        ...
        {/* ── List panel (desktop split / mobile full) ── */}
        {(view === "list" || (view === "split" && !isMobile)) && (
          <div style={{
            position: "absolute", top: 0, left: 0, bottom: 0,
            width: view === "list" ? "100%" : `${SPLIT_PANEL_W}px`,
            overflowY: "auto", background: "#f9fafb",
            borderRight: view === "split" ? "1px solid #e5e7eb" : "none", zIndex: 10,
          }}>
            {filtered.length === 0 ? (
        ...
        {(view === "map" || view === "split") && (
          <div ref={mapDivRef} style={{
            ...
            {/* Enhanced popup card — desktop only */}
            {selected && !isMobile && (
              <div style={{ position: "absolute", top: 16, right: 16, zIndex: 10, width: "320px" }}
                className="bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden animate-fade-in">
                <button onClick={() => setSelected(null)} className="absolute top-2 right-2 w-7 h-7 bg-black/50 hover:bg-black/70 rounded-full flex items-center justify-center z-20 min-h-auto">
                ...
                <div className="p-4">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="text-lg font-extrabold text-blue-600">{fmtCurrency(selected.price)}</p>
                      {selected.previousPrice && selected.previousPrice > selected.price && (
                        <div className="flex items-center gap-1 mt-0.5">
                          <TrendingDown className="w-3 h-3 text-green-600" />
                          <span className="text-xs text-gray-400 line-through">{fmtCurrency(selected.previousPrice)}</span>
                          <span className="text-[10px] font-semibold text-green-600">-{Math.round((1 - selected.price / selected.previousPrice) * 100)}%</span>
                        </div>
                      )}
                    </div>
                    <span className="text-[9px] font-semibold px-2 py-0.5 rounded-full text-white shrink-0 mt-0.5 min-h-auto" style={{ background: getDealColor(selected.dealType) }}>
                      {selected.dealType}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center gap-2.5 mt-2 text-xs text-gray-500">
                    ...
                    {selected.parkingSpaces > 0 && <span className="flex items-center gap-0.5"><Car className="w-3 h-3" />{selected.parkingSpaces}</span>}
                  </div>
                  {selected.downPayment && (
                    <p className="text-xs text-gray-500 mt-2">Down payment: <span className="font-semibold text-gray-700">{fmtCurrency(selected.downPayment)}</span></p>
                  )}
                  <a href={`/listings/${selected.id}`}
                    onClick={(e) => { e.preventDefault(); goToListing(selected.id); }}
                    className="mt-3 w-full flex items-center justify-center py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-semibold rounded-lg transition-colors">
                    View Full Listing →
                  </a>
                </div>
              </div>
        ...
        {isMobile && view === "map" && (
          <div
            className="absolute bottom-0 left-0 right-0 z-20 bg-white rounded-t-2xl shadow-2xl transition-all duration-300 map-card-tray"
            style={{
              maxHeight: trayExpanded ? "60vh" : selected ? "200px" : "120px",
          ...
          >
            ...
            <button
              ...
              className="w-full flex flex-col items-center pt-2 pb-1 cursor-grab"
            >
              <div className="w-10 h-1 rounded-full bg-gray-300 mb-1" />
              <div className="flex items-center gap-1 text-[10px] text-gray-400 font-medium">
                {trayExpanded ? <ChevronDown className="w-3 h-3" /> : <ChevronUp className="w-3 h-3" />}
            ...
            {selected && !trayExpanded && (
              <div className="px-3 pb-2">
                <div className="bg-white rounded-xl shadow-md border border-gray-100 overflow-hidden">
                  <div className="flex">
                    <div className="w-28 h-24 shrink-0 bg-gray-100 overflow-hidden">
                      {selected.image ? <img src={selected.image} alt="" className="w-full h-full object-cover" /> : <div className="w-full h-full bg-gray-200" />}
                    </div>
                    <div className="flex-1 p-2.5 min-w-0">
                      <p className="font-bold text-blue-600 text-sm">{fmtCurrency(selected.price)}</p>
                      ...
                      <a href={`/listings/${selected.id}`}
                        onClick={(e) => { e.preventDefault(); goToListing(selected.id); }}
                        className="mt-1.5 inline-flex items-center text-xs font-semibold text-blue-600">
                        View →
                      </a>
                    </div>
                    <button onClick={() => setSelected(null)} className="p-2 self-start min-h-auto">
            ...
            {/* Expanded: scrollable horizontal card list */}
            {trayExpanded && (
              <div className="overflow-y-auto px-3 pb-3 space-y-2" style={{ maxHeight: "calc(60vh - 60px)" }}>
                {filtered.length === 0 ? (
                  <p className="text-center text-sm text-gray-400 py-4">No listings here</p>
                ) : (
                  filtered.map((listing) => (
                    <MapListingCard
                      key={listing.id}
                      ...
                      onClick={() => {
                        ...
                        if (mapRef.current) mapRef.current.flyTo({ center: [listing.lng, listing.lat], zoom: 14, duration: 600 });
                      }}
                      onHover={() => {}}
                      onLeave={() => {}}
                    />
