...
// price change arrows, "NEW" badges. Homes mode (blue theme).

import maplibregl from "maplibre-gl";
import React, { useEffect, useRef, useState, useCallback } from "react";
import { useNavigate } from "react-router-dom";
import { getAllListings } from "../lib/storage";
import { SlidersHorizontal, X, Bed, Bath, Square, Car, LocateFixed, PenTool, ChevronLeft, ChevronRight, TrendingDown, Sparkles } from "lucide-react";
...
export default function MapSearch() {
  ...
  const [popupPhotoIdx, setPopupPhotoIdx] = useState(0);
  ...
  // ── Build GeoJSON from filtered listings ──
  ...
  useEffect(() => {
    ...
    // Sync DOM markers when map becomes idle (after clustering resolves)
    map.on("idle", () => {
  ...
  // Re-sync markers when filtered changes (source update triggers idle, which calls syncDOMMarkers)
  // But we also need to clear stale markers immediately
  useEffect(() => {
  ...
  // ── Navigate to listing detail (save state first) ──
                ...
                {filtered.map((listing) => (
                  <div key={listing.id} ref={(el) => { listRefs.current[listing.id] = el; }}>
                    <MapListingCard
                      ...
                      onClick={() => { setSelected((p) => p?.id === listing.id ? null : listing); setPopupPhotoIdx(0); if (mapRef.current && listing.lat) mapRef.current.flyTo({ center: [listing.lng, listing.lat], duration: 600 }); }}
            ...
            {selected && (
              <div style={{ position: "absolute", top: 16, right: 16, zIndex: 10, width: "320px" }}
                ...
                <div className="p-4">
                  ...
                  {/* <a href> for middle-click / right-click → open in new tab */}
