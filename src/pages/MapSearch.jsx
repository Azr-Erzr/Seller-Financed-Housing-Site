...
export default function MapSearch() {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      // CRITICAL: call invalidateSize after a short delay so the container
      // has finished painting before Leaflet measures it.
      // Without this, tiles fragment across the screen.
      setTimeout(() => {
        map.invalidateSize();
        setMapReady(true);
      }, 150);
  ...
  // Re-invalidate when the view changes (list→split→map switches container size)
  useEffect(() => {
    ...
    filtered.forEach((listing) => {
      ...
      const icon = Lf.divIcon({
        ...
        html: `<div style="
          ...
          font-weight:700;
          white-space:nowrap;
          box-shadow:${isSelected ? "0 3px 12px rgba(0,0,0,.4)" : "0 2px 8px rgba(0,0,0,.25)"};
  ...
  useEffect(() => {
    ...
    const onMove = (e) => {
      ...
      state.tempRect = Lf.rectangle(bounds, { color: "#059669", fillOpacity: 0.08, weight: 2, dashArray: "6 4" }).addTo(map);
    };
    const onUp = (e) => {
      if (!state.active || !state.startLatLng) return;
      const bounds = Lf.latLngBounds(state.startLatLng, e.latlng);
  ...
  const locateMe = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setLocating(false); alert("Could not get your location. Please allow location access in your browser."); }
    );
  };
  ...
  const clearDraw = () => {
    if (drawLayerRef.current && mapInstance.current) {
      mapInstance.current.removeLayer(drawLayerRef.current);
  ...
  const clearFilters = () => {
    ...
    clearRadius(); clearDraw();
  };

  const activeFilterCount =
    (pricePreset > 0 ? 1 : 0) + dealTypes.length + propTypes.length +
    (minBeds > 0 ? 1 : 0) + (radiusActive ? 1 : 0) + (drawnBounds ? 1 : 0);

  const mapHeight = "calc(100vh - 73px - 48px)"; // viewport minus navbar minus top bar
        ...
        <div className="flex items-center gap-2 ml-1 flex-wrap">
          ...
          {radiusActive && (
            <>
              ...
              <button onClick={clearRadius} className="p-1 hover:bg-gray-100 rounded-full">
                <X className="w-3.5 h-3.5 text-gray-400" />
              </button>
            </>
          )}

          <button onClick={() => { if (!drawMode) { setDrawMode(true); } }}
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${
              drawMode ? "bg-green-600 text-white" :
              drawnBounds ? "bg-green-100 text-green-700" :
          ...
          {drawnBounds && (
            <button onClick={clearDraw} className="p-1 hover:bg-gray-100 rounded-full">
              <X className="w-3.5 h-3.5 text-gray-400" />
            </button>
          )}
        </div>
      ...
      <div className="flex flex-1 overflow-hidden relative">
                  ...
                  <button key={listing.id}
                    onClick={() => {
                      ...
                      if (mapInstance.current && listing.lat) {
                        mapInstance.current.panTo([listing.lat, listing.lng], { animate: true, duration: 0.4 });
                      }
                    }}
        ...
        {(view === "map" || view === "split") && (
          <div className="flex-1 relative overflow-hidden">
            {/* THE FIX: explicit pixel height instead of h-full, which can resolve to 0 */}
            <div
              ref={mapRef}
              style={{ width: "100%", height: mapHeight, minHeight: 400 }}
            />
            ...
            {selected && (
              <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-[1000] w-72 bg-white rounded-2xl shadow-2xl border border-gray-100 overflow-hidden">
                ...
                {selected.image && (
                  <img src={selected.image} alt="" className="w-full h-32 object-cover" />
                )}
                <div className="p-4">
