...
// Same position:absolute layout fix as MapSearch.jsx
...
export default function BusinessMapSearch() {
  ...
  useEffect(() => {
    ...
    if (drawnBounds && leafletRef.current) r = r.filter((l) => drawnBounds.contains([l.lat,l.lng]));
    setFiltered(r);
  ...
  useEffect(() => {
    ...
    import("leaflet").then((L) => {
      ...
      const map = Lf.map(mapDivRef.current, { center:[43.89,-78.93], zoom:9 });
    ...
    return () => {
      if (mapRef.current) { mapRef.current.remove(); mapRef.current = null; leafletRef.current = null; setMapReady(false); }
    };
  }, [view]);
  ...
  useEffect(() => {
    ...
    filtered.forEach((listing) => {
      ...
      const marker = Lf.marker([listing.lat,listing.lng],{icon}).addTo(map)
  ...
  useEffect(() => {
    ...
    if(radiusActive&&userLat!=null){
      radiusLayerRef.current=Lf.circle([userLat,userLng],{radius:radiusKm*1000,color:"#059669",fillColor:"#059669",fillOpacity:0.07,weight:2}).addTo(map);
  ...
  useEffect(() => {
    ...
    const s=drawState.current; s.active=true; map.getContainer().style.cursor="crosshair";
    const onDown=(e)=>{s.startLL=e.latlng;map.dragging.disable();};
    const onMove=(e)=>{if(!s.startLL)return;if(s.tempRect)map.removeLayer(s.tempRect);s.tempRect=Lf.rectangle(Lf.latLngBounds(s.startLL,e.latlng),{color:"#059669",fillOpacity:0.08,weight:2,dashArray:"6 4"}).addTo(map);};
    const onUp=(e)=>{if(!s.startLL)return;const b=Lf.latLngBounds(s.startLL,e.latlng);if(drawLayerRef.current)map.removeLayer(drawLayerRef.current);drawLayerRef.current=Lf.rectangle(b,{color:"#059669",fillOpacity:0.07,weight:2}).addTo(map);setDrawnBounds(b);setDrawMode(false);s.startLL=null;map.dragging.enable();};
            ...
            <div className="absolute left-0 top-0 bottom-0 w-72 bg-white shadow-xl z-30 overflow-y-auto p-5 space-y-5">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {PRICE_PRESETS.map((p,i)=>(<button key={i} onClick={()=>setPricePreset(i)} className={`px-3 py-1.5 rounded-lg text-xs font-medium ${pricePreset===i?"bg-emerald-600 text-white":"bg-gray-100 text-gray-600 hover:bg-gray-200"}`}>{p.label}</button>))}
                </div>
              </div>
              <div>
          ...
          <div ref={mapDivRef} style={{position:"absolute",top:0,right:0,bottom:0,left:view==="split"?"320px":"0"}}>
            {drawMode&&(
              <div style={{position:"absolute",top:12,left:"50%",transform:"translateX(-50%)",zIndex:1000}}
                className="bg-emerald-600 text-white text-xs font-semibold px-4 py-2 rounded-full shadow-lg pointer-events-none">
