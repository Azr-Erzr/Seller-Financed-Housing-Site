...
export default function ListHome() {
  ...
  const [form, setForm] = useState({
    ...
    propertyType: "Single-Family", dealType: "Seller-Finance",
  ...
  if (submitted) {
    return (
      <div className="max-w-lg mx-auto py-20 px-6 text-center">
        ...
        <p className="text-gray-500 mb-8">Your home is now live on HomeMatch. Buyers whose profile matches your terms will be able to find it.</p>
  ...
  return (
    <div className="max-w-3xl mx-auto py-10 px-6">
      <div className="mb-8">
        ...
        <p className="text-gray-500 text-sm">Tell buyers about your property and the financing terms you're offering. Takes about 5 minutes.</p>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-6 space-y-5">
            ...
            <div className="flex flex-wrap gap-4 mt-1">
              {DEAL_TYPES.map((t) => (
                <label key={t} className="flex items-center gap-2 cursor-pointer">
                  <input type="radio" name="dealType" checked={form.dealType === t} onChange={() => set("dealType", t)} className="accent-blue-600" />
                  <span className="text-sm text-gray-700">{t}</span>
                </label>
              ))}
            </div>
          </Field>
          <Field label="Minimum Down Payment ($)" error={errors.downPayment} hint="The minimum you'd accept.">
          ...
          <Field label="Document Privacy">
            <label className="flex items-start gap-3 cursor-pointer mt-1">
              <input type="checkbox" checked={form.docsLocked} onChange={(e) => set("docsLocked", e.target.checked)} className="mt-0.5 accent-blue-600" />
              <div>
                <p className="text-sm font-medium text-gray-700">Require NDA before sharing documents</p>
                <p className="text-xs text-gray-400">Buyers must sign a short NDA before accessing financial details.</p>
              </div>
            </label>
          </Field>
        </div>
