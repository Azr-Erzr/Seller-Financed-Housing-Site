...
export default function Listings() {
  ...
  const clearFilters = () => {
    setLocation(""); setMaxPrice(1000000); setDealTypes([]); setPropTypes([]); setMatchMin(0);
  };
          ...
          <aside className="lg:w-72 shrink-0">
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 sticky top-24">
              ...
              <div className="mb-6">
                ...
                <input type="text" placeholder="City, State, or ZIP" value={location} onChange={(e) => setLocation(e.target.value)}
              ...
              <div className="mb-6">
                ...
                <input type="range" min={200000} max={1200000} step={10000} value={maxPrice} onChange={(e) => setMaxPrice(Number(e.target.value))} className="w-full accent-blue-600" />
                ...
                <div className="space-y-2.5">
                  {DEAL_TYPES.map((type) => (
                    <label key={type} className="flex items-center gap-2.5 cursor-pointer">
                      <input type="checkbox" checked={dealTypes.includes(type)} onChange={() => toggle(dealTypes, setDealTypes, type)} className="w-4 h-4 rounded border-gray-300 accent-blue-600" />
              ...
              <div className="mb-6">
                ...
                <div className="space-y-2.5">
                  {PROPERTY_TYPES.map((type) => (
                    <label key={type} className="flex items-center gap-2.5 cursor-pointer">
                      <input type="checkbox" checked={propTypes.includes(type)} onChange={() => toggle(propTypes, setPropTypes, type)} className="w-4 h-4 rounded border-gray-300 accent-blue-600" />
                      <span className="text-sm text-gray-600">{type}</span>
                    </label>
                  ))}
                </div>
