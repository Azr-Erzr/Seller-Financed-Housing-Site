...
import { money, Badge, Stat, LockValue } from "../ui/UIComponents";
import { useToast } from "../components/Toast";
import NDA from "../components/NDA";
import { Bed, Bath, Square, MapPin, Bookmark, BookmarkCheck, Phone } from "lucide-react";
...
export default function ListingDetail() {
  ...
  useEffect(() => {
    ...
    getAllProfiles().then((profiles) => {
      // We'll set matched profiles once listing loads too
    });
  }, [id]);
  ...
  useEffect(() => {
    if (!listing) return;
    getAllProfiles().then((profiles) => {
      setMatchedProfiles(rankProfilesForListing(profiles, listing).filter(({ score }) => score > 0));
    });
  }, [listing]);
  ...
  const handleSave = () => {
    ...
    toast[nowSaved ? "success" : "info"](
      nowSaved ? "Listing saved to your bookmarks." : "Listing removed from bookmarks."
    );
  };

  const handleContact = () => {
    toast.info("Contact feature coming soon — create a profile to get notified when messaging launches.");
  };
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          ...
          <div className="mt-5 pt-4 border-t">
            <p className="text-sm text-gray-500 mb-3">
              {docsUnlocked
                ? "📄 Documents unlocked. Request full financials from the seller."
                : "📄 Full documents (title, financials) available after signing a short NDA."}
            </p>
            {!docsUnlocked ? (
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        ...
        <div className="grid sm:grid-cols-3 gap-6 mb-6">
          <div>
            ...
            <input type="range" min={listing.downPayment} max={Math.round(listing.price*0.5)} step={1000}
